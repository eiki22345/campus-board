...
Route::get('university-request', [UniversityRequestController::class, 'create'])
  ->middleware('throttle:20,1')->name('university-request.create');
Route::post('university-request', [UniversityRequestController::class, 'store'])
  ->middleware('throttle:3,1')->name('university-request.store');

Route::get('/legal', function () {
  return view('legal.index');
})->middleware('throttle:30,1')->name('legal.index');
...
Route::middleware(['auth', 'verified'])->group(function () {
  ...
  Route::controller(ThreadController::class)->group(function () {
    Route::get('/boards/{board}/threads', 'index')->middleware('throttle:60,1')->name('threads.index');
    Route::get('/boards/{board}/threads/{thread}', 'show')->middleware('throttle:60,1')->name('threads.show');
    Route::post('/boards/threads/{board}', 'store')->middleware('throttle:5,1')->name('threads.store');
    Route::post('/threads/{thread}/like', 'toggleLike')->middleware('throttle:40,1')->name('threads.like');
    Route::delete('/boards/{board}/threads/{thread}', 'destroy')->middleware('throttle:5,1')->name('threads.destroy');
  });
  ...
  Route::controller(PostController::class)->group(function () {
    ...
    Route::delete('/posts/{post}', 'destroy')->middleware('throttle:5,1')->name('posts.destroy');
  });
  ...
  Route::controller(UserController::class)->group(function () {
    Route::get('/users/mypage', 'mypage')->middleware('throttle:60,1')->name('mypage');
    Route::get('/users/edit', 'edit')->middleware('throttle:60,1')->name('users.edit');
